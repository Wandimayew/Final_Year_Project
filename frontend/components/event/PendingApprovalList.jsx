// components/event/PendingApprovalList.jsx
"use client";
...
const cancelAnnouncement = ({ schoolId, token, announcementId, rejectionReason }) =>
  axios.put(
...
const PendingApprovalList = () => {
  ...
  const [confirmation, setConfirmation] = useState(null); // { action, announcementId, rejectionReason }
  ...
  const handleApprove = (announcementId) => setConfirmation({ action: "approve", announcementId });
  const handleCancel = (announcementId) => setConfirmation({ action: "cancel", announcementId, rejectionReason: "" });
  ...
  if (!auth) return <div className="text-center text-red-500 p-6">Please log in.</div>;
    ...
    <div className="min-h-screen bg-gray-100 py-12 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        {pendingAnnouncements.length === 0 ? (
          <p className="text-center text-gray-500 text-lg">No pending announcements.</p>
        ) : (
              ...
              <AnnouncementCard
                ...
                isProcessing={
                  (approveMutation.isLoading &&
                    approveMutation.variables?.announcementId === a.announcementId) ||
                  (cancelMutation.isLoading &&
                    cancelMutation.variables?.announcementId === a.announcementId)
                }
                isAdmin
              />
            ))}
        ...
        {confirmation && (
          <ConfirmationModal
            ...
            title={`Confirm ${confirmation.action === "approve" ? "Approval" : "Cancellation"}`}
            ...
            setReason={(value) => setConfirmation({ ...confirmation, rejectionReason: value })}
          />
        )}
      </div>
    </div>
