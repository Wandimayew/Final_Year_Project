  ...
  const generateQRCode = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8083/staff/api/qrcodes/generate",
        {
      ...
      const ethiopianStartTime = convertToEthiopianTime(
        startHours,
        startMinutes
      );
      const ethiopianEndTime = convertToEthiopianTime(endHours, endMinutes);
      setQrCodeData({ ...response.data, ethiopianStartTime, ethiopianEndTime });
      setSchoolId("");
      setStartTime("");
      setEndTime("");
    } catch (error) {
