...
export default function MarkEntryForm({ onFilter }) {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      setLoading(true);
      try {
        const [streamResponse, classResponse] = await Promise.all([
          axios.get("http://localhost:8081/api/streams"), // Adjust URL
          axios.get("http://localhost:8082/api/classes"), // Adjust URL
        ]);
        setStreams(
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
      <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
        <PencilIcon className="h-6 w-6 text-gray-600" /> {/* Added icon */}
        Mark Entries
      </h2>
