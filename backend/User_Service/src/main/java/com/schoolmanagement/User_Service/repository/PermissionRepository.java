...
public interface PermissionRepository extends JpaRepository<Permission, Long> {
    ...
    @Query("SELECT p FROM Permission p WHERE p.name = :name AND p.schoolId = :schoolId AND p.is_active = true")
    ...
    @Query("SELECT p FROM Permission p WHERE p.schoolId = :schoolId AND p.is_active = true")
    List<Permission> findBySchoolId(@Param("schoolId") String schoolId);

    // Check if a permission name already exists within a school
    @Query("SELECT COUNT(p) > 0 FROM Permission p WHERE p.name = :name AND p.schoolId = :schoolId AND p.is_active = true")
    ...
    @Query("SELECT p FROM Permission p WHERE p.endpoint = :endpoint AND p.httpMethod = :httpMethod AND p.schoolId = :schoolId AND p.is_active = true")
